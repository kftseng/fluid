...
class ObjectAccess {
	...
	static public function getProperty($subject, $propertyName, $forceDirectAccess = FALSE) {
		...
		throw new \TYPO3\Fluid\Reflection\Exception\PropertyNotAccessibleException('The property "' . $propertyName . '" on the subject was not accessible.', 1263391473);
	}
	...
	static public function getPropertyInternal($subject, $propertyName, $forceDirectAccess, &$propertyExists) {
		...
		if ($forceDirectAccess === TRUE) {
			if (property_exists(get_class($subject), $propertyName)) {
				$propertyReflection = new \TYPO3\Fluid\Reflection\PropertyReflection(get_class($subject), $propertyName);
				return $propertyReflection->getValue($subject);
			} elseif (property_exists($subject, $propertyName)) {
				return $subject->{$propertyName};
			} else {
				throw new \TYPO3\Fluid\Reflection\Exception\PropertyNotAccessibleException('The property "' . $propertyName . '" on the subject does not exist.', 1302855001);
			}
		}
