 ...
 * @api
 */
class Service implements \TYPO3\Fluid\Object\SingletonInterface {
	 ...
	 * @return \TYPO3\Fluid\Reflection\ClassSchema
	 ...
	 * @return \TYPO3\Fluid\Reflection\MethodReflection the method Reflection object
	 */
	protected function getMethodReflection($className, $methodName) {
		if (!isset($this->methodReflections[$className][$methodName])) {
			$this->methodReflections[$className][$methodName] = new \TYPO3\Fluid\Reflection\MethodReflection($className, $methodName);
			$this->dataCacheNeedsUpdate = TRUE;
		}
	...
	protected function saveToCache() {
		if (!is_object($this->dataCache)) {
			throw new \TYPO3\Fluid\Reflection\Exception('A cache must be injected before initializing the Reflection Service.', 1232044697);
		}
		$data = array();
		$propertyNames = array(
			'reflectedClassNames',
