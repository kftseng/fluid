...
abstract class AbstractFormFieldViewHelper extends \TYPO3\Fluid\ViewHelpers\Form\AbstractFormViewHelper {
	...
	protected function getLastSubmittedFormData() {
		...
		$value = \TYPO3\Fluid\Reflection\ObjectAccess::getPropertyPath($this->controllerContext->getRequest()->getOriginalRequest()->getArguments(), $propertyPath);
		return $value;
	}
	...
	protected function addAdditionalIdentityPropertiesIfNeeded() {
		...
		if (count($propertySegments) >= 2) {
			...
			for ($i = 1; $i < count($propertySegments); $i++) {
				$object = \TYPO3\Fluid\Reflection\ObjectAccess::getPropertyPath($formObject, implode('.', array_slice($propertySegments, 0, $i)));
	...
	protected function isObjectAccessorMode() {
		return $this->hasArgument('property') && $this->viewHelperVariableContainer->exists('TYPO3\\Fluid\\ViewHelpers\\FormViewHelper', 'formObjectName');
	}
