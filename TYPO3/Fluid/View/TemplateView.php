...
class TemplateView extends \TYPO3\Fluid\View\AbstractTemplateView {
	...
	public function __construct() {
		$this->injectTemplateParser(\TYPO3\Fluid\Compatibility\TemplateParserBuilder::build());
		$this->injectObjectManager(new \TYPO3\Fluid\Object\ObjectManager());
		$this->setRenderingContext($this->objectManager->create('TYPO3\\Fluid\\Core\\Rendering\\RenderingContextInterface'));
	}
	...
	protected function expandGenericPathPattern($pattern, $bubbleControllerAndSubpackage, $formatIsOptional) {
		...
		do {
			...
			$results[] = str_replace('@format', $this->controllerContext->getRequest()->getFormat(), $temporaryPattern);
			if ($formatIsOptional) {
				$results[] = str_replace('.@format', '', $temporaryPattern);
			}
